...
trait ScafiPortableNetworkBinding extends PortableRuntime:
  self: PortableTypes =>
  ...
  trait BindingInterface extends Interface:
    self: ADTs =>
    ...
    ): ConnectionOrientedNetworkManager[ID] = SocketNetworkManager.withFixedNeighbors(
      deviceId,
      port.refineUnsafe,
      neighbors.view.mapValues(e => ADTsIso.endpointIso.get(e)).toMap,
    )
...
end ScafiPortableNetworkBinding
