...
/** A tree-like structure that maps sequences of nodes to values.
  ...
  */
trait ValueTree[N, +V]
    extends Iterable[(Seq[N], V)]
    with PartialFunction[Seq[N], V]:
  ...
  /** Checks if the tree contains the given sequence of nodes as a prefix of
    * some sequence.
    * @param seq
    *   the sequence of nodes
    * @return
  ...
  /** Changes the path of every value in the tree by mapping path-value pairs to
    * new path-value pairs.
    * @param f
    *   the mapping function
    * @tparam N1
    *   the type of the new nodes
  ...
  /** Filters the tree by keeping only the values that satisfy the given
    * predicate.
    * @param f
  ...
  /** Filters the tree by keeping only the values that do not satisfy the given
    * predicate.
    * @param f
    *   the filtering function
    * @return
  ...
  /** Flattens the tree by mapping each path-value pair to a sequence of new
    * path-value pairs.
    * @param f
    *   the mapping function
    * @tparam N1
  ...
  /** Removes all the sequences of nodes that start with the given sequence from
    * the tree.
    * @param seq
    *   the sequence of nodes
    * @return
