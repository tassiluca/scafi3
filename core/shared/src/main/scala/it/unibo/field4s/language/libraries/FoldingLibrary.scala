...
object FoldingLibrary:
  ...
  extension [T](using language: AggregateFoundation)(
      av: language.AggregateValue[T]
  )

    /** Fold (accumulate) the aggregate values using the given function,
      * including the value for self (if present).
      * @param base
      *   the base value
      * @param f
