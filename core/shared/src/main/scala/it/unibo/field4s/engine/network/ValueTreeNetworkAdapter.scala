  ...
  override def send(e: Export[DeviceIdB, ValueTree[TokenB, ValueB]]): Unit =
    network.send(
      e
        ...
        .mapValues(
          _.map((path, value) =>
            path.map(tokenAdapter.backward) -> valueAdapter.backward(value)
          )
        )
    )
  ...
  override def receive(): Import[DeviceIdB, ValueTree[TokenB, ValueB]] = network
    ...
    .map(
      deviceIdAdapter.forward(_) ->
        _.map((path, value) =>
          path.map(tokenAdapter.forward) -> valueAdapter.forward(value)
        )
    )
...
object ValueTreeNetworkAdapter:
  ...
  extension [DeviceIdA, TokenA, ValueA](
      network: Network[DeviceIdA, ValueTree[TokenA, ValueA]]
  )
