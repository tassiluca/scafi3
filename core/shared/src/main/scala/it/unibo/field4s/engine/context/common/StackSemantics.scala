...
/** Implements the semantics of tracing the location of function invocation with
  * a stack and provides a current path for aligning with inbound value trees.
  */
trait StackSemantics:
  ...
  /** @return
    *   the current path of invocation coordinates, that is the path of the
    *   location of the current function scope.
    */
  protected def currentPath: IndexedSeqView[InvocationCoordinate] =
    stack.view.reverse

  /** Executes the given function body inside a new scope, updating the current
    * path accordingly. Restores the previous path after the body has been
    * executed.
    * @param key
