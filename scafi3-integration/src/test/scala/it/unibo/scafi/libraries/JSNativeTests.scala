...
class JSNativeTests extends NativeTests with JSTests:
  ...
  "Exchange aggregate program with branch restriction" should "correctly spread local values to aligned neighbors" in:
    ...
    .futureValue should contain theSameElementsAs Seq(
      0 -> fieldRepr(default = true, neighbors = Map(0 -> true, 2 -> true)),
      1 -> fieldRepr(default = false, neighbors = Map(1 -> false, 3 -> false)),
      2 -> fieldRepr(default = true, neighbors = Map(0 -> true, 2 -> true)),
      3 -> fieldRepr(default = false, neighbors = Map(1 -> false, 3 -> false)),
    )
